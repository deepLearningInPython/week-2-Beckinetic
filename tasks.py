...
def dR(beta, x, y):
    N = x.shape[0]
    dbeta_0 =  (2 / N) * np.sum(beta[0] * np.ones(N) + np.multiply(beta[1], x) - y)# implement the above formula for dR/dβ₀
    dbeta_1 =  (2 / N) * np.sum(np.multiply((beta[0] * np.ones(N) + np.multiply(beta[1], x) - y), x))# implement the above formula for dR/dβ₁
